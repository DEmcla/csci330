# Week 0 — GitHub Onboarding Assignment
...
**Due:** Day 2–3 of Week 1 — see Canvas for the exact date.
...
assignments count. Last term, most lost points were lost to GitHub mistakes, not
to C++. We fix that here, first.
...
3. Create a branch: `git checkout -b week00-onboarding`
4. **Copy this template** into your own week folder (replace `First_Last` with your name):
   ```bash
   cp -r assignment-templates/HW_01 assignments/CSCI330_First_Last/week00
   ```
5. In `assignments/CSCI330_First_Last/week00/`, complete the `TODO` markers in
...
8. **Open a Pull Request** in your fork (`week00-onboarding` → `main`).
...
- A diff that contains **only** files inside `assignments/CSCI330_First_Last/week00/`.

If the CI is red, or the diff reaches outside your week folder, the submission is
not complete — fix it and push again (the PR updates itself). See
